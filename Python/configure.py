...
def create_optparser():
    ...
    p.add_option("-r", "--trace", action="store_true", default=False,
            ...
            "enabled")
    p.add_option("-s", action="store_true", default=False, dest="not_dll",  # TODO check how it works
            help="Fresh is a static library and not a DLL (Windows only)")
    p.add_option("-u", "--debug", action="store_true", default=False,
            help="build the Fresh bindings with debugging symbols")
    p.add_option("-v", "--sipdir", action="callback", default=pyqt.pyqt_sip_dir,
...
def generate_code():
    ...
    makefile = Makefile(
        qt = ['QtCore', 'QtGui', 'QtNetwork', 'QtXml'],
        configuration=pyqt,
    ...
    makefile.extra_include_dirs.append(opts.freshincdir + '/../src')
    makefile.extra_include_dirs.append(opts.freshincdir + '/../build/release/ui') # FIXME bad code
    makefile.extra_lib_dirs.append(opts.freshlibdir)
    makefile.extra_lib_dirs.append('/../')
    makefile.extra_libs.append("fresh")
...
def main(argv):
    ...
    if pyqt4 is None:
        sipconfig.error("PyQt v4 doesn't seem to be installed.")
    else:
        pyqt = pyqt4.Configuration()
        qt_data_dir = pyqt.qt_data_dir
    ...
    if opts.freshmoddir is None:
        ...
        if not os.path.exists(opts.freshmoddir):
            ...
            if os.path.exists(opts.freshmoddir):
                print 'Detected:', os.path.dirname(PyQt4.__file__)
            else:
