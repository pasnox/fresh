...
QTreeWidgetItem* pTranslationDialog::newItem( const QLocale& locale )
{
    ...
    const QString countryCode = locale.name().section( QL1C( '_' ), 1 );

    QTreeWidgetItem* item = new QTreeWidgetItem;
    item->setIcon( 0, pIconManager::icon( QSL( "%1.png" ).arg( countryCode.toLower() ), QSL( ":/fresh/country-flags" ) ) );
    item->setText( 0, QSL( "%1 (%2)" ).arg( language ).arg( country ) );
